...
LayoutPageBase.prototype = {
    ...
    modulesInit: function () {
        ...
        function pageHeroModule(module) {
            var waveColor = $(module).hasClass("tan-bg") ? "#F7F1EC" : "#ffffff";
            var wave = $(module).find(".wave").wavify({
                height: 50,
                bones: 3,
                amplitude: 50,
                color: waveColor,
        ...
        function fullWidthGalleryModule(module) {
            var tl1 = new TimelineLite();
            // var $content = $(module).find(".content-wrapper > *");
            // var $galleryWrapper = $(module).find(".gallery-wrapper");
            // var $captionWrapper = $(module).find(".captions-wrapper");

            
            var galleryOptions = {
                contain: true,
                pageDots: false,
                arrowShape: "M59.8,89l-38-38l38-38l6.3,6.3L34.4,51l31.7,31.7L59.8,89z",
                wrapAround: true,
                imagesLoaded: true,
                on: {
                    change: function(index) {
                       //$currentSlide.text(index + 1);
                       $blurredGallery.flickity("next");
                    }
                }
            };

            var $gallery = $(module).find(".gallery").flickity(galleryOptions);
            ...
            var $blurredGallery = $(module).find(".blurred-gallery").flickity(blurredGalleryOptions);
        }
        ...
        function tiledGalleryModule(module) {
            ...
            if($("html").hasClass("is-ie")) {
                var $img = $(module).find(".bg-img.img-wrapper img");
                objectFitImages($img);
            }

            var scene = new ScrollMagic.Scene({triggerElement: module, offset: loc.scrollOffset, reverse: false})
            .on("start", function (e) {
                tl1.staggerFromTo($content, .5, { opacity: 0, y: 8, ease: Back.easeOut }, { opacity: 1, y: 0, ease: Back.easeOut }, .2, "+=0");
            })
            .addTo(loc.revealController);
        }
        ...
        function pointsOfInterestModule(module) {
            ...
            $.when(
                $.get("https://api.tiles.mapbox.com/mapbox-gl-js/v0.49.0/mapbox-gl.css", function(css) {
                    _css = css;
                }),
                $.getScript("https://api.tiles.mapbox.com/mapbox-gl-js/v0.49.0/mapbox-gl.js")
            ).then(function() {
                ...
                for(var i = 0; i < categories.length; i++) {
                    ...
                    map.on("mouseenter", category, function (e) {
                        ...
                        popup.setLngLat(e.features[0].geometry.coordinates)
                            .setHTML(e.features[0].properties.contentArea)
                            .addTo(map);
                    });
